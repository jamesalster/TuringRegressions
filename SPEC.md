...
B4|2026-07-16|T9 root cause: `_random_effects` (model.jl) correlated-ranef branch (`ranef.has_intercept & ranef.has_fixed_effects`) had a free `intercept_ranef` param acting as an extra mean-shift on the random-slope component: `ranef_matrix = vcat(0, intercept_ranef) .+ diagm(σ)*L*z_raw`. That mean-shift is additively confounded with the population-level fixed effect `β` in `_linear_model` (only `β+intercept_ranef` is identified, since `Xscaled_z ≈ X_scaled` for the same predictor) — a classic ridge in posterior geometry. Same redundancy existed in the slope-only-no-intercept branch (`elseif ranef.has_fixed_effects`), hidden because that testset (test/runtests.jl:374) never asserted fixef recovery, only structural checks under `quickfit!`. Author had already flagged doubt inline (`#ideally with 0 included?`).|V20 — added. Fix: removed `intercept_ranef` from both branches; ranef components are now mean-zero by construction (population α/β already model the mean), matching how the intercept column already used `vcat(0,...)`. Also dropped the now-dead `prior_fixef` arg from `_random_effects`. Verified: sleepstudy correlated ranef testset now recovers Intercept 251.6 vs lme4's 251.4 (was 276.9, off by 25.5) at ~5x faster sampling for the same total draw count (was 13m10s @ 4000 draws, now ~150s @ 4000 draws). AD backend swap (ForwardDiff/ReverseDiff/Mooncake) investigated as a possible additional win — no real difference (apparent ReverseDiff edge was pure JIT-warmup order artifact; Mooncake genuinely ~9x slower) — not pursued further, no adtype option added to `fit!`.

## §T TASKS

T1|~|Two-pass test run: (1) all fits incl benchmark ones cheap (`BENCH_N`/`BENCH_NCHAINS` env-driven, default 300/2) — suite must run error-free, accuracy asserts may fail; (2) re-up benchmark fits to N=2000, nchains via `TR_BENCH_NCHAINS` env override maxed to `Threads.nthreads()` (was fixed 4, too slow) — fix only small/obvious benchmark-tolerance misses, no rebuild. **Parked 2026-07-16**: pass 1 clean (B1 fixed), pass 2 found + fixed B2 (NegativeBinomial crash), but sleepstudy correlated RE testset failure is not a small/obvious miss (see T9) — blocked on T9, then T11 to verify the rest of the suite. **T9 unblocked 2026-07-16 (see B4)** — full suite re-run at N=1000/nchains=4 (moderate, not yet the full N=2000/max-threads budget): 78/79 pass, `Random effects — sleepstudy` 19/19, only failure is the pre-existing unrelated `Weighted fit (T10, V14)` testset tracked at T12. Still open: re-run once at the actual N=2000/nchains=maxed budget to close pass-2 for real, and confirm T11's "rest of suite" clean end-to-end at that budget too|V2,V14,I.plots,T9,T11,B4
...
T8|x|MERGED into T3 (2026-07-16) — co-dependent w/ standardisation rework, both rewrite the same extraction code. See T3 part (b)/(c)

T9|x|Investigate: full-budget NUTS on sleepstudy RE model much slower + biased coefficients on the correlated `(1+Days|Subject)` variant. **Root-caused + fixed 2026-07-16 — see B4/V20**: not a centering, AD-backend, or MCMCThreads-parallelism issue (all four hypotheses from the original investigate list checked; only (a)-adjacent — actual bug was an unidentified free parameter, not centered-vs-noncentered — and (c) AD backend, ruled out by direct benchmark). Fix removed the confounded parameter; sleepstudy correlated testset now passes at ~5x faster sampling for equal draw count and correct coefficient recovery. AD backend swap tested (ForwardDiff/ReverseDiff/Mooncake) as a secondary lever — no real win once the identifiability bug was fixed, not pursued|T1,C6,B4,V20
...
T12|x|`Weighted fit (T10, V14)` @testset (test/runtests.jl) currently skipped (T11 diagnostic): same-seed comparison of weighted (weights=ones) vs unweighted model drifts past atol=0.5 under quickfit's cheap N=300 — likely because `_weighted_likelihood`'s `@addlogprob!` code path vs `_likelihood`'s `~` consume RNG differently even with identical seed, not a math bug (weights=1 is mathematically identical to unweighted). Investigate: confirm no real bug in `_weighted_likelihood` (src/model.jl), then either loosen tolerance to match realistic MCMC noise at N=300, compare via overlapping credible intervals instead of point atol, or bump this test's N. Re-enable testset once resolved|V14

T13|.|First `using TuringRegressions` / first `Pkg.test()` pays full TTFX (Turing/DynamicPPL/model-macro compile). Add a `PrecompileTools.@compile_workload` block (small `turing_glm` fit, `N`/`nchains` minimal) in `src/TuringRegressions.jl` to precompile the hot path ahead of time, cutting first-run latency. **Separate, bigger latency source found during T9 (2026-07-16), think about alongside this**: `construct_model` (model.jl:433) `gensym`s a brand-new model type on *every* `turing_glm()` call by design (comment there: avoids AD/dual-number cache corruption when reusing a shared type name across models with different param counts, per B-worthy incident predating this spec). Measured on sleepstudy correlated ranef: 1st `fit!` (compile+sample) 38.1s vs 2nd `fit!` on the *same* compiled type (sample only) 13.1s — ~25s of every single fit is recompilation, not sampling, and repeat fits of the same formula/family/ranef shape never get to reuse it. `PrecompileTools` can't help here since the gensym'd type doesn't exist until runtime. Real fix would be caching compiled model types by a structural key (family + ranef shape + param counts) instead of gensym-per-call — but the cache key must capture everything that made the old shared-name approach unsafe, so this needs its own careful design, not a quick patch|
