...
id|st|task|cites
T1|.|FIRST+BIGGEST: finish mixed model on `random_effects` git branch — branch already ahead of old SPEC text: working correlated LKJ-Cholesky non-centered varying-intercept+slope, multiple grouping factors (`Vector{RandomEffect}`), per-ranef predictor standardisation w/ FULL centering (kept deliberately — user wants centering everywhere for NUTS efficiency; zero-cost since offset corrections below are deterministic not sampled). `MvNormal(μ,σ::Real)` checked live vs Distributions.jl → `cov=σ²*I`, confirmed NOT a bug, matches old `σ^2*I`, no change. Fixes, all src/model.jl unless noted: (1) `_linear_model` hardcodes `group_idx[:,1]` 3 call sites → must be `group_idx[:,$i]`; breaks multiple grouping factors today, every ranef term reads factor-1's index col. (2) uncorrelated slope-only `_random_effects` branch (no ranef intercept, k slopes, e.g. `(0+x1+x2|g)`) generalize to general k: `variance_ranef~filldist(prior,k)`, `intercept_ranef~filldist(prior_fixef,k)`, `ranef_matrix_raw~filldist(Normal(),k,n_groups[i])`, `ranef_matrix=(intercept_ranef .+ variance_ranef.*raw)'` → `(n_groups×k)`, mean baked in (matches correlated-branch convention); today only works by accident for k==1. (3) `_generated_quantities` correlated branch (has_intercept&has_fixed_effects) missing per-group intercept correction from centering: `α_orig[g]=y_std*ranef_matrix[g,1]-dot(predictors_mn,β_orig[:,g])` (Normal/TDist) or `α_orig[g]=ranef_matrix[g,1]-dot(predictors_mn,β_orig[:,g])` (Bernoulli/Poisson/NegBin); w/o it every group's random intercept biased by `dot(predictors_mn,β_orig)`; mirrors existing correct global correction `α_original=y_mean-dot(X_means,β_original)+y_std*α`. (4) `_generated_quantities` slope-only-no-intercept branch: fix 2 undefined-var bugs (`intercept_slopes`—dead, remove, mean now baked in via fix 2; `predictors_orig`→`beta_orig`); generalize to k slopes; centering here still produces deterministic per-group offset `-dot(predictors_mn,β_orig[:,g])` w/ no modeled-intercept row to absorb it — compute it, do NOT surface as fake "effect" to user (see output-shape below: dedicated hidden layer). (5) `fit!` extraction (src/turingregression.jl) rewritten around new output shape below — old flat-DimArray+`dropdims(dims=1)`-assumes-k==1 approach replaced. (6) NEW: extract group-level SDs (σ_z draws) — sampled but never extracted today, real completeness gap (the actual "variance component" a mixed-model pkg must report, cf `MixedModels.VarCorr`); back-transform mirrors α/β: intercept-row sd × `y_std` (Gaussian/TDist) or ×1 (others); slope-row sd × `y_std/predictors_sd[p]` or ×`1/predictors_sd[p]`.
T1|.|OUTPUT SHAPE (supersedes old flat-DimArray plan; brainstormed 4 options, DimStack chosen over: single flat DimArray w/ string-parsed labels [current draft, loses structure]; two-DimArray/bare-Dict split [no DimensionalData-native show/indexing]; one higher-rank DimArray padded across union of all group levels [wasteful, label-collision risk across factors]). `TR.parameters::DimStack` (was `DimArray`; struct field type change, src/turingregression.jl). Layers: `:fixef`—`(param,draw,chain)`, byte-identical content/shape to today's array (α,β[1..p],σ,ν,ϕ); existing GLM-parity tests V3/V4 read this layer, unaffected. One layer per ranef grouping var, named after it (e.g. `:Subject`)—`(effect,group,draw,chain)`; `effect` axis=`[:Intercept,predictor_names...]` (both present) / `predictor_names...` (slopes-only) / `[:Intercept]` (intercept-only); `group` axis=term's raw levels; values=back-transformed corrected `α_orig`/`β_orig` rows. `:<group>_sd`—`(effect,draw,chain)`, back-transformed group SDs from fix (6). `:<group>_corr`—`(effect,effect2,draw,chain)`, only when correlated: full `L*L'` per draw, separate layer (different rank, can't fold into main layer). `:<group>_offset`—`(group,draw,chain)`, only for slope-only-no-intercept terms: deterministic centering correction from fix (4), hidden from user-facing accessors, consumed only by T1b predict.jl. `draw`/`chain` shared across all layers — that's what makes it one DimStack not a loose array bag. Ripple, same T1 pass (DimStack nothing reads = dead weight): src/parametermethods.jl — `fixef`/`coef`/`parameters`/`internals`/`outcome`/`get_parameters` redirect to `:fixef` layer (behavior-preserving); new `ranef(TR,group_var;fun=nothing)` accessor returns that group's main layer; generic display/summary code must skip `_sd`/`_corr`/`_offset`-suffixed layers when listing "effects" (dedicated logic handles those 3, `_offset` must never surface as a modeled effect). src/summary.jl: minimal ranef display for now (print variance components from `:<group>_sd`; full per-group pretty table later, not blocking). Known limitation, not fixed here (see T1c): 2 ranef terms sharing a grouping var collide on DimStack layer name. Files: src/model.jl, src/turingregression.jl, src/parametermethods.jl, src/summary.jl (minimal). TESTS (test/runtests.jl "Random Effects" testset, written now, fails until T1 lands): sleepstudy (RDatasets lme4/sleepstudy) vs `MixedModels.fit` fixef parity atol~0.5 (looser than V3's 0.025 — ranef pt ests are shrinkage/BLUP vs posterior median); 2-grouping-factor test, `ranef()` layers differ appropriately (proves fix 1); round-trip sanity `α_original+dot(x,β_original)≈raw group mean` (proves fix 3/4)|C2,C6,C8,C10,C13,V3,V4
