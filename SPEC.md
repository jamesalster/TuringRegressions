...
T59|.|Nested/interaction grouping is only tested at the FORMULA level — `(1|a/b)` expands to two grouping terms (`:a`, `Symbol("a:b")`) and `(1|a&b)` to one, both asserted in test/runtests.jl, but neither is ever FIT. Unknown whether `fit!` succeeds on them: two grouping terms sharing a base variable trip the "share grouping variable" warning path, and the `a:b` interaction level set is the product of levels (sparse, many groups with 1 row), which may break `n_groups`/`group_idx` construction or just sample badly. Fit one of each on sleepstudy+Batch, and if it errors, fix or document the limitation|V16,T55
...
T60|.|Plots broken — user reports several Makie plots render wrong/error, suspect Makie or DimensionalData update. Suite only smoke-tests them (`@test_nowarn`, no output inspection), so breakage passes CI: already caught `Makie.series(draws(...; collapse=false)[fixef=At([:α])]')` erroring `adjoint not defined for DimArray{Float64,3}` (length-1 dim retained → 3-D), fixed in readme not src. Go through every plot in ext/TuringRegressionsMakieExt.jl (`lineribbon`/`lineribbon!`, `pp_check_dens`, `pp_check_dens_overlay`, `pp_check_hist`) plus every readme plotting example, render each, EYEBALL output — DD dim → axis mapping, category labels, ribbon bands. Pin Makie/DimensionalData compat once fixed|C7,T54,T56
