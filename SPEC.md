...
T36|.|Type-stability of generated model args. `construct_model` signature (model.jl:218-236) takes `n_groups`, `group_idx`, `group_predictors` untyped — `group_predictors::Vector{Matrix}` is abstract-eltype. Untyped args force fresh inference per call shape and may leak dynamic dispatch into the NUTS hot path. Add concrete annotations, verify with `@code_warntype`/`JET` on the evaluated model. Measure both TTFX and warm time on T23 harness — must not regress §G|T23,C6,T35
