...
T24|~|AD backend. No `adtype` set anywhere in src/ → Turing default ForwardDiff. 42 params @ chunk 12 ≈ 4 forward passes/gradient; Stan reverse-mode = 1. Likely most of gap. Sweep `AutoReverseDiff(compile=true)`, `AutoMooncake()`, `AutoEnzyme()` vs baseline. `compile=true` needs no data-dependent branches in hot path — ours resolved at codegen, verify. Then: pick adtype by param count at `turing_glm` time (ForwardDiff wins <~10-20 params, so fixef-only small → forward, ranef → reverse). Expose `adtype` kwarg regardless|T23
