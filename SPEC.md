...
- `Transform` (`src/standardise.jl`) — affine-map constants, computed once per fit, stored on `TR.tf`. Fields all `StatsBase.ZScoreTransform`: `fixef`, `y`, `ranef::Vector` (aligned with `ModelData.Z`), plus unpacked scalars `y_mean::Float64`, `y_scale::Float64` (0.0/1.0 when family doesn't scale y — `ZScoreTransform` leaves `.mean`/`.scale` empty when `center`/`scale=false`, so the scalars avoid branching at every use site). y-scaling gated on `family_spec(family).scales_y`.
- `FamilySpec` (`src/standardise.jl`) — `scales_y::Bool`, `aux_roots::Tuple{Vararg{Symbol}}` (extra sampled scalars, in output order). Single source of truth for family-specific behaviour; `family_spec(family)`.
  - `compute_transform(md::ModelData, family)::Transform` — pure, constants from raw data. Centring gated on `has_intercept` (only an intercept can absorb the mean_x*slope shift on back-transform); scaling always applied.
  - `apply_transform(tf::Transform, md::ModelData)::ModelData` — applies constants → scaled `ModelData`. Wraps internal `apply_transform(fn::Function, tf, md)` where `fn` is `StatsBase.transform` or `reconstruct` — same traversal either direction.
  ...
  - Back-transform of drawn params lives in `unstandardise` (src/unstandardise.jl), reading `tf.*`, OUTSIDE the model, post-fit.
...
- `RegressionPrior` — `intercept`/`fixed_effects`/`random_effects`/`auxiliary`. Passed as runtime model args, not baked into the generated `Expr` — keeps model shape prior-independent (V20).
...
- V10. Standardised→original param recovery lives in `unstandardise` (src/unstandardise.jl) via `coef_map(has_intercept, xt::ZScoreTransform, y_scale::Float64)`, reading `tf::Transform` (`tf.fixef`, `tf.y_scale`, `tf.ranef`). Runs OUTSIDE the model, post-fit.
...
- V20. Priors are passed as separate runtime args to the generated `@model` (`build_model_expr`, model.jl), NOT baked into the `Expr` and NOT bundled into a struct — model shape stays prior-independent. NO model cache exists: `turing_glm` → `construct_model` `eval`s a fresh `gensym`'d model per call (~25s recompile each). Model name MUST be `gensym`'d — DynamicPPL dispatches evaluation on `typeof(f)`, so a shared name leaked Turing's dual-number caches between models of different param counts (BoundsError during sampling). Caching is T59.
...
T56|x|Packaging blockers — done. MIT `LICENSE` added. `[compat]` now covers every dep + `Makie` weakdep + stdlibs + `julia = "1.10"` (untested below 1.12 — T57's matrix proves it). `LogExpFunctions = "0.3, 1"` so it follows upstream once Bijectors/StatsFuns drop their 0.3 cap; it is the only dep not on latest. Tables KEPT — `columntable` is used. Run instructions added at top of test/runtests.jl. User runs the suite|V17
...
T59|.|Model cache. Every `turing_glm` pays ~25s to `eval` a fresh model; an identical spec should reuse the function. Implementation exists on the UNMERGED `model-cache` branch (`src/model_cache.jl`, `cached_construct_model`, `MODEL_CACHE::Dict{Any,Tuple{Function,Expr}}` + `ReentrantLock`, keyed purely structurally on `family` + `ModelData`'s 4 accessor bools + per-ranef `(variable, has_intercept, has_fixed_effects, n_predictors)`). Priors stay out of the key — already runtime args (V20). Decide: merge that branch or rewrite. NB the gensym'd model name (V20) exists to stop DynamicPPL cache leakage across param counts — a cache MUST NOT reintroduce a shared name for structurally different models|V20
